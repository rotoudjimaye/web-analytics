...
function historyDispatch(hash){
    ...
    var hashBase = hashParts[0]; 
    var queryString = hashParts[1];
    // 
    var handler = historyHandlers[hashBase];
    if(handler){
	handler(hashBase, queryString);
    }
}
function addHistoryHandler(path, handler){
    historyHandlers[path] = handler;
}

function historyInit(){
    jQuery.history.init(historyDispatch);
} 